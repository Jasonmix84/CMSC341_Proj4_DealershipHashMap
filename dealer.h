...
class CarDB{
    ...
    ******************************************/
    ...
    bool rehash(int index, Car car, string type, int cap, prob_t probe);

    bool quarterRehash(int count);

    bool finishRehash(bool inserted);

    Car search(Car* table, int index, int cap, prob_t policy, string model, int dealer) const;
};
#endif
